...
namespace CsvToAvro
{
    public class CsvToAvroWriter<T> where T : ISpecificRecord, IDisposable
    {
        ...
        private static void BuildDataFileWriter(string outputFilePath, Mode mode)
        {
            ...
            {
                case Mode.Overwrite:
                    _dataFileWriter = (DataFileWriter<T>) DataFileWriter<T>.OpenWriter(
        ...
        private void BuildFieldMap()
        {
            ...
            List<Field> fields = _avroSchema.Fields;

            foreach (Field field in fields)
            {
                _fieldMap.Add(field.Name, field.Pos);
            }
        }
        ...
        /// </param>
        /// <returns>A CsvToAvroGenericWriter object.</returns>
        public static CsvToAvroWriter<T> CreateFromJson(string jsonSchema, string outputFilePath,
            Mode mode = Mode.Overwrite)
        {
        ...
        /// </param>
        /// <returns>A CsvToAvroGenericWriter object.</returns>
        public static CsvToAvroWriter<T> CreateFromSchema(RecordSchema schema, string outputFilePath,
            Mode mode = Mode.Overwrite)
        {
            return new CsvToAvroWriter<T>(schema, outputFilePath, mode);
        }
        ...
        private T PopulateRecord(T record, string[] fields)
        {
            if (_csvHeaderFields != null)
            {
                for (var i = 0; i < fields.Length; i++)
                {
                    ...
                    if (_fieldMap.ContainsKey(csvFieldName))
                    {
                        int position = _fieldMap[csvFieldName];
